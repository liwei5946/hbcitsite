...
<!--
		$(document).ready(function(){
			...
			$.ajax({
					url :"${pageContext.request.contextPath}/servlet/LmSelectServlet?Condition=1",
					type : 'get',
					contentType: "application/json;charset=utf-8", 
					async:false,
					dataType:'json',
					data:'id='+<%=request.getParameter("id")%>+'&Condition=1'+"&lmLevel="+<%=request.getParameter("lmLevel")%>,
					success :function(mm){
							...
							}else
							{
								...
								$('#id').val(id);
								$('#LmName').val(user);
								$('#lmLevel').val(<%=request.getParameter("lmLevel")%>);
							}
						},
		...
		function saveLm()
		{
			...
			$.ajax({
					...
					type : 'get',
				    data : 'id='+$('#id').val()+'&lmName='+$('#LmName').val()+'&lmLevel='+$('#lmLevel').val()+'&Condition=0',
					success :function(mm){
				...
				<table  width="100%" id="table1" cellspacing="0"
					...
					<tr>
						...
						<td>
							...
							<input type="hidden"  name="lmLevel" id="lmLevel" >
						</td>
					</tr>
					<tr>
						<td  align="center" colspan="2">
							<input type="button" value="保存修改" name="save" id="save" onclick="saveLm();" class="buttonStyle">
						</td>
					</tr>
				</table>
			</div>
