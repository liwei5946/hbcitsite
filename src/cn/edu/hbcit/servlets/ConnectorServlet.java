...
import cn.edu.hbcit.util.FileOperate;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!RequestCycleHandler.isEnabledForFileUpload(request))
			...
			if (!currentDir.exists())
				...
				FileOperate fo = new FileOperate();//liwei 修正上传乱码、文件名设置为随机数(上传日期+随机数 方式)
				upload.setHeaderEncoding("UTF-8");//liwei 修正上传乱码
				try {
					...
					filename = fo.generateRandomFilename() + extension;//liwei 修正上传乱码、文件名设置为随机数(上传日期+随机数 方式)
					//filename = UUID.randomUUID().toString()+"."+extension;//liwei 修正上传乱码、文件名设置为随机数(UUID方式)
					logger.warn("文件“"+oldName+"”成功上传，并被重新命名为“"+filename+"”。");
